...
module.exports = {
    ...
    sortByAscii(obj, signNull) {
        ...
        for (let i in new_param_keys) {
            for (let j in obj) {
                if ((obj[j] != null && obj[j] !== '' && new_param_keys[i] == j) || (signNull)) {   //参数的值为空不参与签名,如果signNull为true则参与
                    new_obj[j] = obj[j];
                }
            }
        }
    ...
    md5(str, encode) {
        return crypto.createHash('md5').update(str, 'utf8').digest(encode||'hex');
    },
    sha1(str, encode) {
        let sum = crypto.createHash('sha1');
        sum.update(str, 'utf8');
        str = sum.digest(encode || 'hex');
        return str;
    },
    hmac(str, encode, secret) {
        return crypto.createHmac('sha256', secret).update(str).digest(encode || 'base64');
    },

    encrypt(str, algorithm, secret, coding){
        if(['sm4'].includes(algorithm)){
            //特殊算法
            switch(algorithm){
                case 'sm4':
    ...
    decrypt(str, algorithm, secret, coding){
        if(['sm4'].includes(algorithm)){
            //特殊算法
            switch(algorithm){
                case 'sm4':
