...
class ConsultModulationService extends Service {
    async createAppConfig({name, description, company}) {
        const {ctx: { model }, app: {mongoose: {Types: { ObjectId }}}} = this;
        const appConfig = await model.AppConfig({
            company,
            name,
            description
        }).save();
    ...
    async listAppConfig({userInfo, name, company_id, page, pageSize, sortField, sortOrder, user_id}) {
        const {ctx: { model }, app: {mongoose: {Types: { ObjectId }}}} = this;
        ...
        if(!userInfo.isAdmin){
            ...
            if(company_id){
                scope = _.intersection(scope, [{hospitalId: company_id}])
            }
            condition["company.hospitalId"] = {
                "$in": scope.map(item=>item.hospitalId)
            };
        }else{
            if(company_id){
