    ...
    async alertBug({app_id, api_id, message}){
        ...
        if(appConfig.qywechat && appConfig.qywechat.alertBot && appConfig.qywechat.alertBot.webhook){
            ...
            let data = {
                "msgtype": "text",
                "text": {
                    "content": `[${time} ${appConfig.company.hospitalName}(${appConfig.company.hospitalId})] 接口: ${apiConfig.name} 调用发生错误， 错误信息: ${message}`,
                    "mentioned_list": (mentioned_list && mentioned_list.length>0) ? mentioned_list : ["@all"]
                }
            }
